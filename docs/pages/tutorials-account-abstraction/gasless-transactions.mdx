...
In this tutorial, you will submit your first fully-gasless transaction from a smart account using the Owl Protocol [ERC4337](https://www.erc4337.io/docs) Account Abstraction API & [permissionless](https://docs.pimlico.io/permissionless)
...
For the purposes of this guide, we will be using [Simple Account](https://github.com/eth-infinitism/account-abstraction/blob/develop/contracts/samples/SimpleAccount.sol) as a smart wallet implementation. This is an ERC-4337 wallet controlled by a single owner (eg. Metamask, Owl Developer Wallet).
...
<Callout>
If you visit the address on the block explorer, you might notice that no contract is actually deployed to this address yet. This is because smart accounts are counterfactual, meaning that they are only deployed on-chain the first time you send a transaction through the account.
</Callout>
