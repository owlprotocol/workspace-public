...
let publicOpenRpcSchema: OpenrpcDocument;
let publicRpcValidator: MethodCallValidator;

/**
 * Memoized function to get public open rpc schema and validator
 */
export async function getPublicOpenRpcSchema() {
    publicOpenRpcSchema = publicOpenRpcSchema ?? (await dereferenceDocument(publicOpenRpcSchema_ as any));
    publicRpcValidator = publicRpcValidator ?? new MethodCallValidator(publicOpenRpcSchema);
