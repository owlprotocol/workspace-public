...
library ERC1155Lib {
    ...
    bytes32 constant ERC1155_STORAGE =
        keccak256(abi.encode(uint256(keccak256("erc1155.storage")) - 1)) & ~bytes32(uint256(0xff));

    /// @custom:storage-location erc7201:erc1155.storage
    ...
    function getData() internal pure returns (ERC1155Storage storage ds) {
        bytes32 position = ERC1155_STORAGE;
        assembly {
            ds.slot := position
        }
    }
    ...
    function _balanceOf(address account, uint256 id) internal view returns (uint256) {
        return getData()._balances[id][account];
    }
    ...
    function _balanceOfBatch(address[] memory accounts, uint256[] memory ids) internal view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < accounts.length; ++i) {
            batchBalances[i] = _balanceOf(accounts.unsafeMemoryAccess(i), ids.unsafeMemoryAccess(i));
        }
    ...
    function _isApprovedForAll(address account, address operator) internal view returns (bool) {
        return getData()._operatorApprovals[account][operator];
    }
    ...
    function _update(address from, address to, uint256[] memory ids, uint256[] memory values) internal {
        ...
        for (uint256 i = 0; i < ids.length; ++i) {
            ...
            if (from != address(0)) {
                uint256 fromBalance = getData()._balances[id][from];
                if (fromBalance < value) {
                    revert IERC1155Errors.ERC1155InsufficientBalance(from, fromBalance, value, id);
                }
                unchecked {
                    // Overflow not possible: value <= fromBalance
                    getData()._balances[id][from] = fromBalance - value;
                }
            }

            if (to != address(0)) {
                getData()._balances[id][to] += value;
            }
        }
