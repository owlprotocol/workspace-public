 ...
 * ```
 */
export type MapType<T, From, To> = T extends From
    ? To
    : T extends (infer R)[]
    ? MapType<R, From, To>[]
    : T extends Record<string, any>
    ? { [K in keyof T]: MapType<T[K], From, To> }
    : T;
...
/**
 * Converts Record<string,never> to Record<never,never>
 */
export type AsEmptyRecord<T> = TypeEqual<Record<string, never>, T> extends true ? Record<never, never> : T;
 ...
 * ```
 */
export declare type TypeEqual<Target, Value> = (<T>() => T extends Target ? 1 : 2) extends <T>() => T extends Value
    ? 1
    : 2
    ? true
    : false;
 ...
 * //   ^? type Result = ['a', 'c']
 */
export type Filter<T extends readonly unknown[], P, Acc extends readonly unknown[] = []> = T extends readonly [
    infer F,
