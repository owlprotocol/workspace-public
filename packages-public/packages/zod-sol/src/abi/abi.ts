...
export function contractFactoryWithZod<T extends Abi = Abi>(
    f: TypechainContractFactory<T>,
): TypechainFactoryWithZod<T> {
...
export type IContractFactoryWithZod<T extends IContractFactory = IContractFactory> = ReturnType<
    typeof contractFactoryWithZod2<T["abi"], T>
>;
export function contractFactoryWithZod2<T extends Abi = Abi, U extends IContractFactory<T> = IContractFactory<T>>(
    f: U,
) {
    const abiFunctions = abiWithFunctionsOnly<T>(f.abi);
