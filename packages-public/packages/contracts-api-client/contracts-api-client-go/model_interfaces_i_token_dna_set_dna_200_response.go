/*
Owl Contract Api

Specification for our API focused on contract interactions

API version: 0.0.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the InterfacesITokenDnaSetDna200Response type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &InterfacesITokenDnaSetDna200Response{}

// InterfacesITokenDnaSetDna200Response struct for InterfacesITokenDnaSetDna200Response
type InterfacesITokenDnaSetDna200Response struct {
	ContractParams InterfacesITokenDnaSetDnaRequestContractParams `json:"contractParams"`
	TxHash string `json:"txHash"`
}

// NewInterfacesITokenDnaSetDna200Response instantiates a new InterfacesITokenDnaSetDna200Response object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewInterfacesITokenDnaSetDna200Response(contractParams InterfacesITokenDnaSetDnaRequestContractParams, txHash string) *InterfacesITokenDnaSetDna200Response {
	this := InterfacesITokenDnaSetDna200Response{}
	this.ContractParams = contractParams
	this.TxHash = txHash
	return &this
}

// NewInterfacesITokenDnaSetDna200ResponseWithDefaults instantiates a new InterfacesITokenDnaSetDna200Response object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewInterfacesITokenDnaSetDna200ResponseWithDefaults() *InterfacesITokenDnaSetDna200Response {
	this := InterfacesITokenDnaSetDna200Response{}
	return &this
}

// GetContractParams returns the ContractParams field value
func (o *InterfacesITokenDnaSetDna200Response) GetContractParams() InterfacesITokenDnaSetDnaRequestContractParams {
	if o == nil {
		var ret InterfacesITokenDnaSetDnaRequestContractParams
		return ret
	}

	return o.ContractParams
}

// GetContractParamsOk returns a tuple with the ContractParams field value
// and a boolean to check if the value has been set.
func (o *InterfacesITokenDnaSetDna200Response) GetContractParamsOk() (*InterfacesITokenDnaSetDnaRequestContractParams, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ContractParams, true
}

// SetContractParams sets field value
func (o *InterfacesITokenDnaSetDna200Response) SetContractParams(v InterfacesITokenDnaSetDnaRequestContractParams) {
	o.ContractParams = v
}

// GetTxHash returns the TxHash field value
func (o *InterfacesITokenDnaSetDna200Response) GetTxHash() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.TxHash
}

// GetTxHashOk returns a tuple with the TxHash field value
// and a boolean to check if the value has been set.
func (o *InterfacesITokenDnaSetDna200Response) GetTxHashOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.TxHash, true
}

// SetTxHash sets field value
func (o *InterfacesITokenDnaSetDna200Response) SetTxHash(v string) {
	o.TxHash = v
}

func (o InterfacesITokenDnaSetDna200Response) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o InterfacesITokenDnaSetDna200Response) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["contractParams"] = o.ContractParams
	toSerialize["txHash"] = o.TxHash
	return toSerialize, nil
}

type NullableInterfacesITokenDnaSetDna200Response struct {
	value *InterfacesITokenDnaSetDna200Response
	isSet bool
}

func (v NullableInterfacesITokenDnaSetDna200Response) Get() *InterfacesITokenDnaSetDna200Response {
	return v.value
}

func (v *NullableInterfacesITokenDnaSetDna200Response) Set(val *InterfacesITokenDnaSetDna200Response) {
	v.value = val
	v.isSet = true
}

func (v NullableInterfacesITokenDnaSetDna200Response) IsSet() bool {
	return v.isSet
}

func (v *NullableInterfacesITokenDnaSetDna200Response) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableInterfacesITokenDnaSetDna200Response(val *InterfacesITokenDnaSetDna200Response) *NullableInterfacesITokenDnaSetDna200Response {
	return &NullableInterfacesITokenDnaSetDna200Response{value: val, isSet: true}
}

func (v NullableInterfacesITokenDnaSetDna200Response) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableInterfacesITokenDnaSetDna200Response) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


