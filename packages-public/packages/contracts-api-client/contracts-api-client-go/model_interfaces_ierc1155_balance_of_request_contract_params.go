/*
Owl Contract Api

Specification for our API focused on contract interactions

API version: 0.0.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the InterfacesIERC1155BalanceOfRequestContractParams type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &InterfacesIERC1155BalanceOfRequestContractParams{}

// InterfacesIERC1155BalanceOfRequestContractParams struct for InterfacesIERC1155BalanceOfRequestContractParams
type InterfacesIERC1155BalanceOfRequestContractParams struct {
	// An ethereum address
	Var0 *string `json:"0,omitempty"`
	// A solidity uint256
	Var1 *string `json:"1,omitempty"`
	// An ethereum address
	Account *string `json:"account,omitempty"`
	// A solidity uint256
	Id *string `json:"id,omitempty"`
}

// NewInterfacesIERC1155BalanceOfRequestContractParams instantiates a new InterfacesIERC1155BalanceOfRequestContractParams object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewInterfacesIERC1155BalanceOfRequestContractParams() *InterfacesIERC1155BalanceOfRequestContractParams {
	this := InterfacesIERC1155BalanceOfRequestContractParams{}
	return &this
}

// NewInterfacesIERC1155BalanceOfRequestContractParamsWithDefaults instantiates a new InterfacesIERC1155BalanceOfRequestContractParams object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewInterfacesIERC1155BalanceOfRequestContractParamsWithDefaults() *InterfacesIERC1155BalanceOfRequestContractParams {
	this := InterfacesIERC1155BalanceOfRequestContractParams{}
	return &this
}

// GetVar0 returns the Var0 field value if set, zero value otherwise.
func (o *InterfacesIERC1155BalanceOfRequestContractParams) GetVar0() string {
	if o == nil || IsNil(o.Var0) {
		var ret string
		return ret
	}
	return *o.Var0
}

// GetVar0Ok returns a tuple with the Var0 field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InterfacesIERC1155BalanceOfRequestContractParams) GetVar0Ok() (*string, bool) {
	if o == nil || IsNil(o.Var0) {
		return nil, false
	}
	return o.Var0, true
}

// HasVar0 returns a boolean if a field has been set.
func (o *InterfacesIERC1155BalanceOfRequestContractParams) HasVar0() bool {
	if o != nil && !IsNil(o.Var0) {
		return true
	}

	return false
}

// SetVar0 gets a reference to the given string and assigns it to the Var0 field.
func (o *InterfacesIERC1155BalanceOfRequestContractParams) SetVar0(v string) {
	o.Var0 = &v
}

// GetVar1 returns the Var1 field value if set, zero value otherwise.
func (o *InterfacesIERC1155BalanceOfRequestContractParams) GetVar1() string {
	if o == nil || IsNil(o.Var1) {
		var ret string
		return ret
	}
	return *o.Var1
}

// GetVar1Ok returns a tuple with the Var1 field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InterfacesIERC1155BalanceOfRequestContractParams) GetVar1Ok() (*string, bool) {
	if o == nil || IsNil(o.Var1) {
		return nil, false
	}
	return o.Var1, true
}

// HasVar1 returns a boolean if a field has been set.
func (o *InterfacesIERC1155BalanceOfRequestContractParams) HasVar1() bool {
	if o != nil && !IsNil(o.Var1) {
		return true
	}

	return false
}

// SetVar1 gets a reference to the given string and assigns it to the Var1 field.
func (o *InterfacesIERC1155BalanceOfRequestContractParams) SetVar1(v string) {
	o.Var1 = &v
}

// GetAccount returns the Account field value if set, zero value otherwise.
func (o *InterfacesIERC1155BalanceOfRequestContractParams) GetAccount() string {
	if o == nil || IsNil(o.Account) {
		var ret string
		return ret
	}
	return *o.Account
}

// GetAccountOk returns a tuple with the Account field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InterfacesIERC1155BalanceOfRequestContractParams) GetAccountOk() (*string, bool) {
	if o == nil || IsNil(o.Account) {
		return nil, false
	}
	return o.Account, true
}

// HasAccount returns a boolean if a field has been set.
func (o *InterfacesIERC1155BalanceOfRequestContractParams) HasAccount() bool {
	if o != nil && !IsNil(o.Account) {
		return true
	}

	return false
}

// SetAccount gets a reference to the given string and assigns it to the Account field.
func (o *InterfacesIERC1155BalanceOfRequestContractParams) SetAccount(v string) {
	o.Account = &v
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *InterfacesIERC1155BalanceOfRequestContractParams) GetId() string {
	if o == nil || IsNil(o.Id) {
		var ret string
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InterfacesIERC1155BalanceOfRequestContractParams) GetIdOk() (*string, bool) {
	if o == nil || IsNil(o.Id) {
		return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *InterfacesIERC1155BalanceOfRequestContractParams) HasId() bool {
	if o != nil && !IsNil(o.Id) {
		return true
	}

	return false
}

// SetId gets a reference to the given string and assigns it to the Id field.
func (o *InterfacesIERC1155BalanceOfRequestContractParams) SetId(v string) {
	o.Id = &v
}

func (o InterfacesIERC1155BalanceOfRequestContractParams) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o InterfacesIERC1155BalanceOfRequestContractParams) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Var0) {
		toSerialize["0"] = o.Var0
	}
	if !IsNil(o.Var1) {
		toSerialize["1"] = o.Var1
	}
	if !IsNil(o.Account) {
		toSerialize["account"] = o.Account
	}
	if !IsNil(o.Id) {
		toSerialize["id"] = o.Id
	}
	return toSerialize, nil
}

type NullableInterfacesIERC1155BalanceOfRequestContractParams struct {
	value *InterfacesIERC1155BalanceOfRequestContractParams
	isSet bool
}

func (v NullableInterfacesIERC1155BalanceOfRequestContractParams) Get() *InterfacesIERC1155BalanceOfRequestContractParams {
	return v.value
}

func (v *NullableInterfacesIERC1155BalanceOfRequestContractParams) Set(val *InterfacesIERC1155BalanceOfRequestContractParams) {
	v.value = val
	v.isSet = true
}

func (v NullableInterfacesIERC1155BalanceOfRequestContractParams) IsSet() bool {
	return v.isSet
}

func (v *NullableInterfacesIERC1155BalanceOfRequestContractParams) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableInterfacesIERC1155BalanceOfRequestContractParams(val *InterfacesIERC1155BalanceOfRequestContractParams) *NullableInterfacesIERC1155BalanceOfRequestContractParams {
	return &NullableInterfacesIERC1155BalanceOfRequestContractParams{value: val, isSet: true}
}

func (v NullableInterfacesIERC1155BalanceOfRequestContractParams) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableInterfacesIERC1155BalanceOfRequestContractParams) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


