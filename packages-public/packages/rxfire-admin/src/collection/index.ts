...
import { Observable, MonoTypeOperatorFunction, OperatorFunction, pipe, UnaryFunction, from } from "rxjs";
import { map, filter, scan, distinctUntilChanged, startWith, pairwise } from "rxjs/operators";
import { fromRef } from "../fromRef.js";
import { snapToData } from "../document/index.js";
import { DocumentChangeType, DocumentChange, Query, QueryDocumentSnapshot, DocumentData } from "../interfaces.js";
import { CountSnapshot } from "../interfaces.js";
 ...
 */
const filterEvents = <T>(events?: DocumentChangeType[]): MonoTypeOperatorFunction<DocumentChange<T>[]> =>
 ...
 * @param change
 */
function processIndividualChange<T>(combined: DocumentChange<T>[], change: DocumentChange<T>): DocumentChange<T>[] {
    switch (change.type) {
        case "added":
...
const windowwise = <T = unknown>() =>
    pipe(startWith(undefined), pairwise() as OperatorFunction<T | undefined, [T | undefined, T]>);
 ...
 */
const filterEmptyUnlessFirst = <T = unknown>(): UnaryFunction<Observable<T[]>, Observable<T[]>> =>
    pipe(
        windowwise(),
...
export function collection<T = DocumentData>(query: Query<T>): Observable<QueryDocumentSnapshot<T>[]> {
    return fromRef(query).pipe(map((changes) => changes.docs));
}
