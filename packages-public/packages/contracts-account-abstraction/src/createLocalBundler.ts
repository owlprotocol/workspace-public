...
import { BundlerRpcSchema } from "permissionless/types/bundler";
...
export function createLocalBundlerClient(
    parameters: PublicClientConfig<Transport, Chain> &
        WalletClientConfig<Transport, Chain, Account> & {
    ...
    const client = createClient({
        ...
        .extend(walletActions) as unknown as PimlicoBundlerClient<ENTRYPOINT_ADDRESS_V07_TYPE>;

    const publicClient = client as unknown as PublicClient<Transport, Chain>;
    const walletClient = client as unknown as WalletClient<Transport, Chain, Account>;
    const chain = publicClient.chain as unknown as Chain;

    const requestDefault = client.request;

    //@ts-expect-error
    const requestOverride: EIP1193RequestFn<BundlerRpcSchema<ENTRYPOINT_ADDRESS_V07_TYPE> & PimlicoBundlerRpcSchema> =
